...
async function makeQueuedSession() {
  ...
  await execFileAsync('git', ['branch', '-M', 'issue-43/runner-contract'], { cwd: repoDir });
  ...
  const started = parseJson<Envelope<{ session_id: string }>>(
    (
      await runCli(repoDir, [
        'session',
        'start',
        'Runner delivery proof',
        '--goal',
        'Prove transition delivery semantics',
        '--issue',
        '#43',
        '--json',
      ])
...
describe('threadloop runner contract', () => {
  it('uses documented protocol-v3 CLI fields and fails closed at human or unknown work', async () => {
    ...
    expect(protocol.data.contractVersions.protocol).toBe(3);
    expect(protocol.data.contractVersions.sessionNext).toBe(3);
    ...
    expect(skill).toContain('runner:v1:<wake_id>:<expected_state_version>');
    expect(skill).toContain('After any transition or required-work action, stop.');
    expect(skill).toContain('On `STATE_VERSION_CONFLICT`, stop.');
    expect(skill).toContain('On `IDEMPOTENCY_CONFLICT`, stop and escalate');
    expect(skill).toContain('All other `required_work`, `guard_failures`, and `next_human_action` codes are unknown');
    expect(skill).toContain('`ready_for_human`, `blocked`, or `completed`');
    expect(skill).toContain('RERUN_CORRUPT_GATES');
    expect(skill).toContain('never reset, clean, stash');
  });

  it('deduplicates exact and lost responses, but rejects a changed request under the same wake key', async () => {
  ...
  it('allows one winner when distinct wakes race from the same state version', async () => {
    const { repoDir, sessionId } = await makeQueuedSession();
