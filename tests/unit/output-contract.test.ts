...
import {
  createCommandFailureEnvelope,
  createCommandSuccessEnvelope,
  renderCommandFailure,
  renderCommandSuccess,
} from '../../src/contracts/output.js';
...
describe('output contract', () => {
  it('renders a stable JSON success envelope', () => {
    ...
    ).toBe(JSON.stringify(createCommandSuccessEnvelope('session start', { session_id: 'session_123' }), null, 2));
  });
  ...
  it('renders stable text failures with hint details', () => {
    ...
    ).toBe('threadloop [SESSION_REQUIRED]: A session id is required for this command.\nHint: Pass --session <id>.');
  });
  ...
  it('renders stable text failures without hints as a single line', () => {
    ...
    ).toBe('threadloop [SESSION_NOT_FOUND]: Could not find active session: session_missing');
  });
});
